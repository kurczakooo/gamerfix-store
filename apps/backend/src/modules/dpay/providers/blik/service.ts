...
class DpayBlikProviderService extends AbstractPaymentProvider<Options> {
  static identifier = "dpay_blik";
  ...
  static validateOptions(options: Record<any, any>): void {
    if (!options.apiKey) {
      throw new Error("Dpay BLIK provider requires an apiKey option");
    }
  }
...
export default DpayBlikProviderService;
